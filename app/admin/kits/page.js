    ...
    <div className="space-y-5 animate-fade-in">
      <div>
        ...
        <p className="text-brand-navy/60 mt-1">
          {rows.length} total · showing latest 200
        </p>
      </div>
      ...
      <Card className="!p-4">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search by location, type, or user email…"
            value={search}
            ...
            <tbody>
              {loading && (
                <tr>
                  <td
                    colSpan={7}
                    className="px-4 py-8 text-center text-brand-navy/50"
                  >
                    Loading…
                  </td>
                </tr>
                ...
                <tr key={k.id} className="border-t border-brand-navy/5">
                  <td className="px-4 py-3">
                    <p className="font-semibold text-brand-navy">
                      {k.user_profiles?.full_name || "—"}
                    </p>
                    <p className="text-xs text-brand-navy/60">
                      {k.user_profiles?.email}
                    </p>
                  </td>
                  <td className="px-4 py-3">
      ...
      <Modal
        ...
        title={viewing ? `Kit — ${viewing.location}` : ""}
        size="xl"
      >
        {viewing && (
          <div className="space-y-4">
            <div className="flex flex-wrap gap-2">
              ...
              {viewing.target_audience && (
                <Badge variant="blue" className="capitalize">
                  {viewing.target_audience}
                </Badge>
              )}
              {viewing.tone && (
                <Badge className="capitalize">{viewing.tone}</Badge>
              )}
              {viewing.language && (
                <Badge variant="gray">{viewing.language}</Badge>
              )}
            </div>
            {SECTIONS.map((s) => (
              <div key={s.key}>
                <h4 className="font-bold text-brand-navy mb-2">{s.label}</h4>
                <div className="kit-text bg-brand-cream/40 rounded-xl p-4 border border-brand-navy/5 text-sm">
                  {viewing[s.key] || "—"}
                </div>
              </div>
            ))}

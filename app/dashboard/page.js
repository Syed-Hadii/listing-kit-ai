...
export default async function DashboardHome() {
  ...
  const [{ data: profile }, { data: recentKits }, { data: paymentReq }] =
    await Promise.all([
      supabase
        .from("user_profiles")
        .select(
          "full_name, current_plan, subscription_status, credits_remaining, credits_used, total_kits_generated",
        )
        .eq("id", user.id)
        .single(),
      supabase
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          ...
          <p className="text-brand-navy/60 mt-1">
            Here's your Listing Kit AI workspace.
          </p>
        </div>
        ...
        <div className="card p-5 border-red-200 bg-red-50 flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-red-600 mt-0.5">
              Upgrade your plan to continue generating marketing kits.
            </p>
          </div>
        ...
        <div className="card p-5 border-amber-200 bg-amber-50 flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="font-bold text-amber-800">
              Low credits: only {profile.credits_remaining} left
            </p>
            <p className="text-sm text-amber-700 mt-0.5">
              Top up your plan before your next listing.
            </p>
          </div>
          <Link href="/dashboard/billing" className="btn btn-primary btn-sm">
            Upgrade
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
            {recentKits.map((k) => (
              <Link
                ...
                className="card p-5 hover:border-brand-gold/40 transition-colors"
              >

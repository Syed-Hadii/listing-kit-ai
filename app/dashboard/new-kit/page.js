...
export default function NewKitPage() {
  ...
  function copyFull() {
    if (!kit) return;
    const combined = SECTION_META.map(
      (s) => `## ${s.label}\n\n${kit[s.key]}`,
    ).join("\n\n---\n\n");
    copy(combined, "full");
  }
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-brand-navy/60 mt-1">
          Fill in the property details. One generation uses 1 credit.
        </p>
      </div>
      ...
      {!kit && (
        <Card className="w-full">
          <form onSubmit={onGenerate} className="space-y-5">
            ...
            <Textarea
              label="Key features"
              placeholder="Pool, smart home, chef's kitchen, ocean view…"
              value={form.key_features}
              onChange={(e) => update("key_features", e.target.value)}
            />
            <Textarea
              label="Additional property description"
              placeholder="Anything else the AI should know about this listing."
              value={form.property_description}
              onChange={(e) => update("property_description", e.target.value)}
            />
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <Select
                ...
                onChange={(e) => update("platform_focus", e.target.value)}
              >
                {PLATFORMS.map((p) => (
            ...
            <div className="pt-2 flex justify-end">
              <Button type="submit" variant="gold" size="lg" disabled={loading}>
                <Sparkles size={16} />{" "}
                {loading ? "Generating…" : "Generate Kit (1 credit)"}
              </Button>
            </div>
          </form>
        </Card>
      ...
      {loading && !kit && (
        <Card>
          <div className="flex flex-col items-center py-12">
            ...
            <p className="mt-4 font-display text-xl text-brand-navy">
          ...
          <div className="card p-5 bg-emerald-50 border-emerald-200 flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="font-bold text-emerald-800">
                Kit generated and auto-saved
              </p>
              <p className="text-sm text-emerald-700">
                Find it anytime inside Saved Kits.
              </p>
            </div>
            <Button
              variant="outline"
              onClick={copyFull}
              className="border-emerald-300 text-emerald-700 hover:text-emerald-900"
            >
            ...
            <Card key={s.key}>
              <div className="flex items-center justify-between mb-3">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => copy(kit[s.key], s.key)}
                >
                  {copied === s.key ? <Check size={14} /> : <Copy size={14} />}
                  {copied === s.key ? "Copied" : "Copy"}
                </Button>
              </div>
              <div className="kit-text bg-brand-cream/40 rounded-xl p-4 border border-brand-navy/5">
                {kit[s.key]}
              </div>
            </Card>
          ))}

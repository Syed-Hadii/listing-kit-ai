...
export default function KitDetailPage() {
  ...
  if (loading)
    return <div className="card text-center text-brand-navy/60">Loading…</div>;
  if (!kit)
    return (
      <div className="card text-center text-brand-navy/60">Kit not found.</div>
    );
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <Link
        href="/dashboard/kits"
        className="inline-flex items-center gap-1 text-sm text-brand-navy/60 hover:text-brand-navy"
      >
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              {kit.language && (
                <Badge className="bg-white/10 text-white">{kit.language}</Badge>
              )}
            </div>
            ...
            <p className="text-white/70 mt-1">
              {kit.price} · Created {formatDate(kit.created_at)}
            </p>
          </div>

...
export default function SavedKitsPage() {
  ...
  const types = useMemo(() => [...new Set(kits.map((k) => k.property_type).filter(Boolean))], [kits]);

  const filtered = useMemo(() => {
    let out = [...kits];
    if (search) out = out.filter((k) => (k.location || "").toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl md:text-4xl font-bold text-brand-navy">Saved Kits</h1>
          <p className="text-brand-navy/60 mt-1">{kits.length} kit{kits.length !== 1 ? "s" : ""} in your library</p>
        </div>
        ...
        <Card className="!p-4">
          <div className="grid md:grid-cols-3 gap-3">
            <div className="relative">
              ...
              <input className="input pl-10" placeholder="Search by location…" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
      ...
      {loading ? (
        ...
        <EmptyState
          icon={Filter}
          title={kits.length === 0 ? "No saved kits yet" : "No kits match your filters"}
          description={kits.length === 0 ? "Generate your first kit to see it here." : "Try clearing the search or filter."}
          ...
          {filtered.map((k) => (
            <Card key={k.id} className="!p-5 hover:border-brand-gold/40 transition-colors group">
              ...
              <Link href={`/dashboard/kits/${k.id}`} className="block">
                ...
                {k.target_audience && (
                  <p className="text-xs text-brand-navy/50 mt-1 capitalize">{k.target_audience} · {k.tone}</p>
                )}
              </Link>
              <div className="flex gap-2 mt-4">
                <Link href={`/dashboard/kits/${k.id}`} className="btn btn-primary btn-sm flex-1">View</Link>
                <Button variant="danger" size="sm" onClick={() => onDelete(k.id)}>
                  <Trash2 size={14} />
                </Button>
              </div>
            </Card>

...
export function Sidebar({ variant = "user", userRole = "user" }) {
  ...
  const content = (
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 border-t border-white/10">
        <button
          onClick={onLogout}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-white/70 hover:bg-white/5 hover:text-white"
        >
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className="lg:hidden fixed top-0 inset-x-0 z-30 bg-brand-navy h-20 flex items-center justify-between px-4 border-b border-white/10">
        <img
          src="/images/logo.png"
          alt="Listing Kit AI"
